...
class Xlsdatapenyuplai extends CI_Controller {
    function __construct(){
     ...
     public function export(){
        $ambildata = $this->model_xls->export_penyuplai();
        ...
        if(count($ambildata)>0){
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('B1', "IDENTITAS PENYUPLAI");
            ...
            $val = array("No.","Kode Penyuplai","Nama","Alamat","No Telpon");
            for ($a=0;$a<5; $a++) 
            {
            ...
            foreach ($ambildata as $frow){
                ...
                $objset->setCellValue("B".$baris, $frow->kd_penyuplai);
				$objset->setCellValue("C".$baris, $frow->nama_penyuplai);
                $objset->setCellValue("D".$baris, $frow->alamat_penyuplai); 
                $objset->setCellValue("E".$baris, $frow->notelp_penyuplai);
            ...
            $filename = urlencode("Data_Penyuplai_Tanggal_".date("Y-m-d_H_i_s").".xls");
